...
function Spell:CastQ(Unit)
    ...
    if --DMW.Time > CastTimer and  and
    self:IsReady() and (Unit.Distance <= self.MaxRange or IsSpellInRange(self.SpellName, Unit.Pointer) == 1) and (Unit.Player or Unit.TTD >= self:CastTime()) then
        CastTimer = DMW.Time   -- + (math.random(110, 170) / 1000)
        if DMW.Player.Moving then
            SendMovementUpdate()
        end
